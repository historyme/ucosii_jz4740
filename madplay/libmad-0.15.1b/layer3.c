 ...
 *
 * $Id: layer3.c,v 1.1.1.1 2007/08/23 02:32:16 jgao Exp $
 */
...
mad_fixed_t III_requantize(unsigned int value, signed int exp)
{
  ...
  if (exp < 0) {
    ...
    if (exp >= 5) {
      /* overflow */
# if defined(DEBUG)
     printf("requantize overflow (%f * 2^%d)\n",
...
# if 0 && defined(DEBUG)
  if (bits_left < 0)
    printf("read %d bits too many\n", -bits_left);
  else if (cachesz + bits_left > 0)
    printf("%d stuffing bits\n", cachesz + bits_left);
# endif
...
# if 0 && defined(DEBUG)
  printf( "main_data_begin:%u, md_len:%u, frame_free:%u, "
