...
static void  DrawBitLine18BPP(int x, int y, U32 const * p, int xsize, const LCD_PIXELINDEX * pTrans) {

	printf("DrawBitLine18BPP\r\n");
...
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
  ...
  /* Read from hardware ... Adapt to your system */
  {
      ...
      tOff Off = XY2OFF(x,y);	  
	  READ_MEM(Off,Data);
	 // printf("sizeof = %d 0x%x\r\n",sizeof(LCD_PIXELINDEX),Data);
      //PixelIndex = Data;/* ... */
  }
  return Data;
}
                       ...
                       const LCD_PIXELINDEX* pTrans)
{
  ...
  for (i=0; i<ysize; i++) {
	...
	#if (LCD_BITSPERPIXEL > 16)
      case 18:
        DrawBitLine18BPP(x0, i + y0, (const U32 *)pData, xsize, pTrans);
        break;
    #endif	
    }
    pData += BytesPerLine;
