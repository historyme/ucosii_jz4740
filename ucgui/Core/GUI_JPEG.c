...
static void _Initex(struct jpeg_error_mgr* pjerr, struct jpeg_decompress_struct *pcinfo, FILE *infile) {
  /* 1. Allocate and initialize a JPEG decompression object. */
  pcinfo->err = jpeg_std_error(pjerr);
	jpeg_create_decompress(pcinfo);
  /* 2. Init the source manager so the library can retrieve data via methods */
...
int GUI_JPEG_GetInfoex(FILE *infile, GUI_JPEG_INFO* pInfo) {
	struct jpeg_decompress_struct cinfo;
	struct jpeg_error_mgr jerr;
  _Initex(&jerr, &cinfo, infile);
  /*
   * Release the JPEG decompression object.
   */
  jpeg_destroy_decompress(&cinfo);
...
int GUI_JPEG_Draw(const void * pFileData, int DataSize, int x0, int y0) {
  ...
  _Init(&jerr, &cinfo, pFileData, DataSize);
    ...
    if (!hBuffer) {
	   ...
	   // hBuffer = GUI_ALLOC_AllocNoInit(cinfo.output_width * 3);
    }
    while (cinfo.output_scanline < cinfo.output_height) {
      ...
      if (cinfo.jpeg_color_space == JCS_GRAYSCALE) {
	      ...
	      _WritePixelsGray(p, x0, y0 + cinfo.output_scanline, cinfo.output_width);
      } else {
	      // _WritePixelsRGB(p, x0, y0 + cinfo.output_scanline, cinfo.image_width);
	       _WritePixelsRGB(p, x0, y0 + cinfo.output_scanline, cinfo.output_width);
      }
    }
...
int GUI_JPEG_Drawex(FILE * infile, int x0, int y0) {
  #if (GUI_WINSUPPORT)
    GUI_RECT r;
  #endif
  int Ret = 0;
...
*       GUI_JPEG_GetInfo
*/
int GUI_JPEG_GetInfoEx(FILE * infile, int DataSize, GUI_JPEG_INFO* pInfo) {
	struct jpeg_decompress_struct cinfo;
	struct jpeg_error_mgr jerr;
  /*
   * Release the JPEG decompression object.
...
int GUI_JPEG_DrawExA(FILE * infile, int x0, int y0,int cx,int cy) {
  #if (GUI_WINSUPPORT)
    GUI_RECT r;
  #endif
  int Ret = 0;
