...
static int _AD2X(int adx) {
  ...
  r *= GUI_TOUCH_XSIZE - 1;
  //printf("r = %d %d\n",r,xyMinMax[GUI_COORD_X].Max - xyMinMax[GUI_COORD_X].Min);
...
static void Calibrate(unsigned short *dat)
{
	int x,y;
	xPhys = *dat;
	yPhys = *(dat + 1);
	//printf("x,y = %x,%x\r\n",xPhys,yPhys);
	//printf("xMin,xMax = %x,%x\r\n",xMin,xMax);
	//printf("yMin,yMax = %x,%x\r\n",yMin,yMax);
	//printf("TOUCH DIM = %x,%x\r\n",GUI_TOUCH_XSIZE,GUI_TOUCH_YSIZE);
...
void GUI_TOUCH_SetDefaultCalibration(void) {
	...
	//TS_SetxCalibration(xCalibrate,yCalibrate);
