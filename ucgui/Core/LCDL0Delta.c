...
*       _DrawBitLine18BPP
*/
static void _DrawBitLine18BPP(int x, U32 const GUI_UNI_PTR * p, int xsize, const LCD_PIXELINDEX * pTrans) {
  int xx = x;
  LCD_PIXELINDEX pixel;
  printf(" _DrawBitLine18BPP\r\n");
  if ((GUI_Context.DrawMode & LCD_DRAWMODE_TRANS) == 0) {
    if (pTrans) {
      for (; xsize > 0; xsize--, xx++, p++) {
        pixel = *p;
  ...
  for (i = 0; i < ysize; i++) {
    ...
    case 1:
      ...
      case 16:
	   ...
	   case 18:
           _DrawBitLine18BPP(x0, (const U32 *)pData, xsize, pTrans);
        break;
	#endif
    }
    pData += BytesPerLine;
