...
static struct {
...
} __attribute__ ((packed)) confDesc = {
	{
		...
		0x64	// Maximum power consumption 200 mA
		//0x00
	},
	{
		sizeof(USB_InterfaceDescriptor),
...
static inline unsigned int USB_HandleCBW(unsigned int handle)
{
	...
	case UFI_READ_CAPACITY:
		Handle_UFI_READ_CAPACITY(handle);
		break;
	case UFI_READ_10:
...
static unsigned int massreset(unsigned int handle)
{
	...
	massbuf = (unsigned char *)((unsigned int)massBuf | 0xa0000000);
...
static unsigned int Set_UDC_Speed(unsigned stat)
{
	...
	case UDC_HIGHSPEED:
        ...
        //devDesc.bcdUSB = 0x200;
		break;
		
	case UDC_FULLSPEED:
		printf("UDC FULLSPEED\r\n");
		//devDesc.bcdUSB = 0x110;
		break;
...
static inline unsigned int usbHandleClassDevReq(unsigned int handle,unsigned char *buf)
{
	...
	switch (buf[1]) {
	case 0xfe:
		scsiLUN = devDesc.iSerialNumber;
		dprintf("Get max lun %d\n",scsiLUN);
		if (buf[0] == 0xa1)
