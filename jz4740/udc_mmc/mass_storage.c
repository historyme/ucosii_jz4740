...
//			USB_EndpOut(1, (const u8*)resp, sizeof(resp), false);
			...
			//resp[0] = swap32(499711);
			//resp[0] = swap32(MMC_ReadCapacity()-2);  
		...
		case UFI_WRITE_AND_VERIFY:
		{
			...
			if (cbw.CBWCB[0] == UFI_READ_10)  //PC read SD card
			{
				u16 i,j,k;
				i = nr_sectors / 16;
				j = nr_sectors % 16;
				...
				for (k=0;k<i;k++)
				{
					MMC_ReadMultiBlock(start_sector,16, massBuf);//read SD
					if (!Enable_DMA_Write(massBuf,4096*2)) return;
					start_sector += 16;
				}
				if (j)
				{
					MMC_ReadMultiBlock(start_sector,j,massBuf);
					if (!Enable_DMA_Write(massBuf,j*512)) return;
				}
				//printf("\n SEnd 13!!");
			...
			{       //PC write SD card
				...
				for (k=0;k<i;k++)
				{
					if (!Enable_DMA_Read(massBuf,4096*2)) return;
					//if (start_sector==0)MMC_WriteMultiBlock(start_sector+1,7,massBuf+512);
					//else 
					//__dcache_writeback_all();
					//__dcache_invalidate_all();
					MMC_WriteMultiBlock(start_sector,16,massBuf);
					start_sector += 16;
				}
				if (j)
				{
					if (!Enable_DMA_Read(massBuf,j*512)) return ;
					//if (start_sector==0)MMC_WriteMultiBlock(start_sector+1,j-1,massBuf+512);
					//else 
					//__dcache_writeback_all();
					//__dcache_invalidate_all();
					MMC_WriteMultiBlock(start_sector,j,massBuf);
				}
				//HW_SendPKT(epin, &csw, sizeof(CSW));
				massStat = S_CSW;
			}
...
//		FS_IoCtl(USR_DISK, FS_CMD_FLUSH_CACHE, 0, 0);
	...
	case S_DATA_OUT:
		if (!hwTxFifoCount()) {
			...
			if (nr_sectors > 8) {
				...
				MMC_ReadMultiBlock(start_sector,8, massBuf);//read SD
				HW_SendPKT(epin, massBuf, 8*512);
				start_sector += 8;
				nr_sectors -= 8;
			} else {
				//massDevRead(cbw.bCBWLUN,massBuf, start_sector, nr_sectors);
				MMC_ReadMultiBlock(start_sector,nr_sectors, massBuf);//read SD
	...
	case S_DATA_IN:
		...
		if ((nr_sectors >= 8) && (tmp >= 4096)) {
			...
			MMC_WriteMultiBlock(start_sector,8,massBuf);
			start_sector += 8;
			nr_sectors -= 8;
			...
			MMC_WriteMultiBlock(start_sector,nr_sectors,massBuf);
			start_sector += nr_sectors;
			nr_sectors = 0;
		}
		if (nr_sectors == 0)
